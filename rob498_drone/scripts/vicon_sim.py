...
class ViconSim:
    def __init__(self, link_name):
        ...
        self.link_state_sub = rospy.Subscriber("/gazebo/link_states", LinkStates, self.link_state_sub_callback)
    ...
    def link_state_sub_callback(self, data):
        try:
            link_pose = data.pose[data.name.index(self.link_name)]
            self.link_transform_stamped = pose_to_transform_stamped(link_pose)
        except ValueError:
    ...
    def run(self):
        while not rospy.is_shutdown():
            ...
            self.tf_pub.sendTransform(self.link_transform_stamped)
            self.rate.sleep()
