...
def pose_error(
    ...
    if full_pose:
        ...
        rot_diff = (angle + np.pi) % (2 * np.pi) - np.pi
        rot_error = np.abs(rot_diff)
    else:
        pose_1 = pose_to_drone_pose(pose_1)
        pose_2 = pose_to_drone_pose(pose_2)

        pos_diff = pose_2[0:3] - pose_1[0:3]
        pos_error = np.linalg.norm(pos_diff)

        rot_diff = pose_2[3] - pose_1[3]
        rot_diff = np.arctan2(np.sin(rot_diff), np.cos(rot_diff))
        rot_error = np.abs(rot_diff)
        

    return pos_error, rot_error, pos_diff, rot_diff
